...
class SavedRecipeDetails extends StatefulWidget {
  final RecipeEntry recipeEntry;
  const SavedRecipeDetails({Key? key, required this.recipeEntry}) : super(key: key);

  @override
  _SavedRecipeDetailsState createState() => _SavedRecipeDetailsState(recipeEntry);
}

class _SavedRecipeDetailsState extends State<SavedRecipeDetails> {
  final RecipeEntry recipeEntry;
  _SavedRecipeDetailsState(this.recipeEntry);
  ...
  void pushEditEntry(BuildContext context) {
    Navigator.push(context, MaterialPageRoute(builder: (context) => EditRecipe(entryData: recipeEntry))).then((data) {
      setState(() => {});
    });
  }
  ...
  Widget verifyDeleteRecipe(BuildContext context) {
    return AlertDialog(
        ...
        actions: <Widget>[
          ...
          TextButton(
            onPressed: () {
              profileProcessor.removeFromLibrary(recipeEntry.id);
              setState(() {});
              Navigator.of(context).pop();
              Navigator.of(context).pop();
              Navigator.of(context).pop();
